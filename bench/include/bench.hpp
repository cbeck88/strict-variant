...
struct assignment_helper<T, ilist<is...>> {
  ...
  template <typename U>
  static void assign(T & t) { t = U{}; }

  static constexpr std::array<ftype, sizeof...(is)> funcs() {
   return {{ (&assign<dummy<is>>)... }};
  }
};

template <template<class...> class variant_template, uint32_t N>
void set_type(dummy_variant_t<variant_template, N> & v, uint32_t idx) {
  static constexpr auto funcs = assignment_helper<dummy_variant_t<variant_template, N>, Count_t<N>>::funcs();
  funcs[idx % N](v);
}
