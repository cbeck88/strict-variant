...
struct visitor_applier {
  ...
  uint32_t operator()(T && t) const {
    return juice::apply_visitor(benchmark::dummy_visitor{}, std::forward<T>(t));
  }
};

int
main() {
  benchmark::run_benchmark<juice::variant, num_variants, seq_length, repeat_num, visitor_applier>(
