...
auto eval_visitor_impl(Visitor && vis, std::tuple<Args...> && tup, mpl::ulist<us...>) -> decltype(std::forward<Visitor>(std::declval<Visitor>())(std::forward<Args>(std::declval<Args>())...)) {
...
struct multivisitor_state<Visitor, TypeList<Vs...>, TypeList<>> : multivisitor_state_base<Visitor, TypeList<Vs...>, TypeList<>> {
  ...
  decltype(std::forward<Visitor>(std::declval<Visitor>())(std::forward<Vs>(std::declval<Vs>())...)) evaluate() {
    return eval_visitor_impl(std::forward<Visitor>(vis_), std::move(vs_), mpl::count_t<sizeof...(Vs)>{});
  }
};

// Inductive hypothesis
// Clang really wants the functor to be a complete type at the time that we are evaluating "evaluate", so these need
...
struct multivisitor_state_functor<Visitor, TypeList<Vs...>, TypeList<U1, Us...>> : multivisitor_state_base<Visitor, TypeList<Vs...>, TypeList<U1, Us...>> {
  ...
  auto operator()(T && t) -> decltype(std::declval<next_state_t<T>>().evaluate()) {
    next_state_t<T> new_state{std::forward<Visitor>(vis_), std::tuple_cat(vs_, std::tuple<T>{std::forward<T>(t)}), mpl::cdr_tuple(us_)};
    return new_state.evaluate();
  }
};

template <typename Visitor, typename... Vs, typename U1, typename... Us>
struct multivisitor_state<Visitor, TypeList<Vs...>, TypeList<U1, Us...>> : multivisitor_state_functor<Visitor, TypeList<Vs...>, TypeList<U1, Us...>> {
  ...
  auto evaluate() -> decltype(apply_visitor(*static_cast<functor_t*>(nullptr), std::forward<U1>(std::declval<U1>()))) {
...
auto multivisit_impl(Visitor && visitor, Us && ... us) -> decltype(static_cast<mpl::multivisitor_state<Visitor, mpl::TypeList<>, mpl::TypeList<Us...>> *>(nullptr)->evaluate()) {
  mpl::multivisitor_state<Visitor, mpl::TypeList<>, mpl::TypeList<Us...>> st{std::forward<Visitor>(visitor), {}, std::tuple<Us...>{std::forward<Us>(us)...}};
  return st.evaluate();
}
...
auto apply_visitor(Visitor && vis, V1 && v1, V2 && v2, Vs && ... vs) -> decltype(detail::multivisit_impl(std::forward<Visitor>(std::declval<Visitor>()), std::forward<V1>(std::declval<V1>()), std::forward<V2>(std::declval<V2>()), std::forward<Vs>(std::declval<Vs>())...)) {
  return detail::multivisit_impl(std::forward<Visitor>(vis), std::forward<V1>(v1), std::forward<V2>(v2), std::forward<Vs>(vs)...);
}
